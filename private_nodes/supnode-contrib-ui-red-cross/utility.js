...
const checkConfig = (config, node, RED) => {
    if (!config) {
        ...
        node.error(RED._('ui_red_cross.error.no-config'));
        return false;
    }
    if (!config.group) {
        node.error(RED._('ui_red_cross.error.no-group'));
        return false;
    }
    if (RED.nodes.getNode(config.group) === undefined) {
        node.error(RED._('ui_red_cross.error.invalid-group'));
        return false;
    }
    return true;
};
